...
void maker(void* args) {
    ...
    while(num_CashiersLeft > 0) {
        ...
        thread_signal(lock, currentOrder); //signal that the sandwich is ready
        thread_wait(lock, -2);
    }
    endloops:
    thread_unlock(lock);
...
int main(int argc, char *argv[]) {
    boardMaxSize = atoi(argv[1]);
    num_CashiersLeft = argc - 2;

    char* inputs[num_CashiersLeft];
