...
extern int thread_libinit(thread_startfunc_t func, void *arg) {
  ...
  try {
      deleteThread = new myThread;
      deleteThread->ucontext_ptr = new ucontext_t;
      deleteThread->stack = new char [STACK_SIZE];
      deleteThread->finished = 0;
      deleteThread->threadID = -1;
  } catch (const std::bad_alloc& ba) {
      interrupt_enable();
      return -1;
  }

  if (getcontext(deleteThread->ucontext_ptr) == -1) {
      interrupt_enable();
      return -1;
  }

  deleteThread->ucontext_ptr->uc_stack.ss_sp = deleteThread->stack;
  deleteThread->ucontext_ptr->uc_stack.ss_size = STACK_SIZE;
  deleteThread->ucontext_ptr->uc_stack.ss_flags = 0;
  deleteThread->ucontext_ptr->uc_link = NULL;

  makecontext(deleteThread->ucontext_ptr, (void (*)()) clearThread, 0);
  ...
  if (swapcontext(prev, currThread->ucontext_ptr) == -1) {
      interrupt_enable();
      return -1;
  }
...
static void clearThread() {
    ...
    delete currThread->ucontext_ptr;
    delete currThread;
    setcontext(prev);
}
...
static int start(thread_startfunc_t func, void *arg) {
  ...
  if (swapcontext(currThread->ucontext_ptr, deleteThread->ucontext_ptr) == -1) {
      interrupt_enable();
      return -1;
  }
  return 0;
...
extern int thread_create(thread_startfunc_t func, void *arg) {
  ...
  try {
    ...
    delete newCreate->ucontext_ptr;
    delete newCreate->stack;
    delete newCreate;
    interrupt_enable();
    return -1;
  }
...
extern int thread_yield(void) {
  ...
  if (ready.size() != 0 ) {
    ...
    if (swapcontext(currThread->ucontext_ptr,prev) == -1) {
        interrupt_enable();
        return -1;
    }
  }
...
extern int thread_lock(unsigned int lock) {
  ...
  if (lock_map.find(lock) == lock_map.end()) {
    try {
      ...
      delete newLock->blocked;
      delete newLock->lock_holder;
      delete newLock;
      interrupt_enable();
      return -1;
    }
